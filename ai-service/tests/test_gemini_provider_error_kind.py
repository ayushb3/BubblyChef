...
class TestClassifyHttpError:
    def test_429_with_quota_body_is_quota_exhausted(self) -> None:
        body = '{"error": {"status": "RESOURCE_EXHAUSTED", "message": "exceeded your quota"}}'
        assert _classify_http_error(429, body) == "quota_exhausted"

    def test_plain_429_is_rate_limited(self) -> None:
        body = '{"error": {"status": "TOO_MANY_REQUESTS"}}'
...
class TestCompleteRaisesClassifiedKind:
    ...
    async def test_quota_429_sets_kind_quota_exhausted(self) -> None:
        ...
        error = _http_error(429, '{"error": {"status": "RESOURCE_EXHAUSTED"}}')
        response = MagicMock()
