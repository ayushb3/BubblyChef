...
INTENT_CLASSIFICATION_SYSTEM_PROMPT = (
    ...
    "cooking times (NOT recipe requests). Also covers STOCK QUESTIONS — "
    "asking whether they have an item, not telling you about one — "
    "'do I have spinach?', 'is there any milk left?', "
    ...
    "A bare flavor or mood word with no dish name and no request verb is "
    "still open-ended, even though it reads like a preference rather than "
    "a question — treat it as recipe_brainstorm, not recipe_generation. "
    "This applies ONLY to a flavor/mood on its own; a request for one meal "
    "with constraints (time, difficulty, ingredients on hand) still wants "
    "a specific recipe and stays recipe_generation:\n"
    "- 'I'm craving something spicy' → recipe_brainstorm "
    "(a flavor/mood alone, not a dish or a request)\n"
    "- 'I want something comforting' → recipe_brainstorm\n"
    "- 'give me a spicy chicken curry recipe' → recipe_generation "
    "(names a specific dish)\n"
    "- 'quick easy meal under 30 minutes' → recipe_generation "
    "(asks for one meal, with constraints — not a bare mood)\n\n"
    ...
    "- 'give me a pasta recipe' → recipe_generation\n\n"
    "IMPORTANT: Distinguish cooking_help from pantry_update — a question "
    "ASKING whether an item is on hand is NOT the same as a statement "
    "TELLING you about a pantry change, even though both name a food item:\n"
    "- 'do I have spinach?' → cooking_help\n"
    "- 'is there any milk left?' → cooking_help\n"
    "- 'what cheese do I have?' → cooking_help\n"
    "- 'do we have any eggs?' → cooking_help\n"
    "- 'is there butter in the fridge?' → cooking_help\n"
    "- 'I bought spinach' → pantry_update\n"
    "- 'add 2 eggs' → pantry_update\n"
    "- 'used up the last of the milk' → pantry_update\n\n"
