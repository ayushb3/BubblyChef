...
# Value: (result, inserted_at_monotonic)
_alias_cache: OrderedDict[
    tuple[tuple[str, ...], tuple[str, ...]],
    tuple[dict[str, "ResolvedAlias"], float],
] = OrderedDict()
...
def _alias_cache_get(
    ...
    now: float,
) -> dict[str, "ResolvedAlias"] | None:
    """Return cached aliases if present and not expired; None otherwise."""
    ...
    # Shallow copy so a caller mutating its result cannot corrupt the shared
    # entry for every later cook. ResolvedAlias is a frozen dataclass, so the
    # values are safe to share; only the mapping itself needs protecting.
    return dict(result)


def _alias_cache_put(
    key: tuple[tuple[str, ...], tuple[str, ...]],
    result: dict[str, "ResolvedAlias"],
    now: float,
) -> None:
    """Insert into cache, evicting the LRU entry when full."""
    if key in _alias_cache:
        _alias_cache.move_to_end(key)
    # Store a copy: the caller keeps using the mapping it passed in, and a
    # mutation there must not reach into the shared entry. ResolvedAlias is
    # frozen, so a shallow copy is enough.
    _alias_cache[key] = (dict(result), now)
...
async def resolve_aliases_with_llm(
    ...
    *,
    ...
    Results are cached by (sorted unmatched names, sorted pantry names) for
    _ALIAS_CACHE_TTL seconds with LRU eviction at _ALIAS_CACHE_MAX_SIZE entries.
    ...
    _alias_cache_put(cache_key, aliases, now)
    return aliases
