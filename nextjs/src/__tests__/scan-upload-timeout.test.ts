...
it('#439: maps an abort that fires while the response body is still being read to the same client-timeout ScanError, not a raw AbortError', async () => {
  jest.useFakeTimers()
    ...
    return Promise.resolve({
      ...
      json: () =>
        new Promise((_resolve, reject) => {
          // Simulates the deadline firing mid-parse, after `fetch` itself
          // already resolved — the previous code only wrapped the `fetch`
          // call in an AbortError -> ScanError mapping, so an abort here
          // used to escape as a raw DOMException.
          init?.signal?.addEventListener('abort', () => {
