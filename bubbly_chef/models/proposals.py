...
class HandoffProposal(BaseModel):
    ...
    instructions: str = Field(
        description="User-friendly instructions for what to do next"
    )
    ...
    optional_inputs: list[str] = Field(
        ...
        description="List of optional inputs"
    )
    example_prompt: str | None = Field(
        default=None,
        description="Example of what the user could say/upload"
    )
...
class ChatMessage(BaseModel):
    ...
    metadata: dict[str, Any] = Field(
        ...
        description="Additional metadata (e.g., intent detected, tokens used)"
    )
...
class ReviewEvent(BaseModel):
    ...
    edits: dict[str, Any] | None = Field(
        default=None,
        description="Edited proposal data (for approve_with_edits)"
    )
    feedback: str | None = Field(
        default=None,
        description="Optional user feedback or reason for rejection"
    )
...
class IntentClassification(BaseModel):
    ...
    detected_entities: list[str] = Field(
        default_factory=list,
        description="Key entities detected in the input"
    )
...
class ClarificationRequest(BaseModel):
    ...
    context: str | None = Field(
        default=None,
        description="Context explaining why we're asking"
    )
    ...
    field_reference: str | None = Field(
        default=None,
        description="Which field this question relates to"
    )
    item_index: int | None = Field(
        default=None,
        description="Index of the item in question if applicable"
    )
...
class ParsedPantryItem(BaseModel):
    ...
    confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Per-item confidence from LLM"
    )
...
class GeneralChatResponse(BaseModel):
    ...
    detected_topics: list[str] = Field(
        default_factory=list,
        description="Topics detected in the conversation"
    )
    follow_up_suggestions: list[str] = Field(
        default_factory=list,
        description="Suggested follow-up topics or actions"
    )
