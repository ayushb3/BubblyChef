...
from bubbly_chef.models.base import ProposalEnvelope, Intent
...
async def ingest_product(request: ProductIngestRequest) -> ProposalEnvelope[PantryProposal]:
    ...
    if not request.barcode and not request.description:
        raise HTTPException(
            status_code=400,
            detail="Either barcode or description is required"
        )
    
    logger.info(
        f"Product ingest request: barcode={request.barcode}, "
...
async def ingest_recipe(request: RecipeIngestRequest) -> ProposalEnvelope[RecipeCardProposal]:
    ...
    if not request.url and not request.text:
        raise HTTPException(
            status_code=400,
            detail="Either url or text is required"
        )
