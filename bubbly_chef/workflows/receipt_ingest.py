...
RECEIPT_PARSE_SYSTEM_PROMPT = """You are a helpful assistant that extracts grocery items from receipt OCR text.
...
async def parse_receipt_llm(state: WorkflowState) -> WorkflowState:
    ...
    try:
        ...
        logger.info(f"Receipt LLM parsed {len(parsed_items)} items with confidence {adjusted_confidence}")
