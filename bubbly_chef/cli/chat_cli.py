...
def print_response(envelope: Any, debug: bool = False):
    ...
    if RICH_AVAILABLE:
        ...
        intent_str = envelope.intent.value if hasattr(envelope.intent, 'value') else str(envelope.intent)
        ...
        console.print(Panel(
            ...
            subtitle=f"[dim]intent: {intent_str} | confidence: {envelope.confidence.overall:.0%}[/dim]",
            border_style=intent_color,
        ))
        
        # Show proposal if present
        if envelope.proposal and hasattr(envelope.proposal, 'actions') and envelope.proposal.actions:
            ...
            for action in envelope.proposal.actions:
                conf_style = "green" if action.confidence > 0.8 else "yellow" if action.confidence > 0.6 else "red"
                table.add_row(
        ...
        # Show handoff proposal
            ...
            console.print(f"[dim]Required inputs: {', '.join(envelope.proposal.required_inputs)}[/dim]")
        
        # Show next action hint
        next_action_str = envelope.next_action.value if hasattr(envelope.next_action, 'value') else str(envelope.next_action)
        ...
        if envelope.proposal and hasattr(envelope.proposal, 'actions') and envelope.proposal.actions:
            ...
            for action in envelope.proposal.actions:
                ...
                print(f"  - {action_type}: {action.item.quantity} {action.item.unit} {action.item.name}")
