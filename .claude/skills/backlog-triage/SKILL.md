...
- If not, publish `board.html` with `icon: "checklist"` and these capabilities:
  ...
  Anyone the board is shared with at `interact` level can write decisions, and
  the board doesn't record who wrote one. So the board stays **private to
  Ayush**: never share it. Before step 5, check it is still unshared
  (`Artifact` `action: "read"` shows its sharing). If it has been shared, list
  the decisions in chat and apply only after Ayush confirms them there.
...
| `close` | Comment the evidence (and `Duplicate of #<n>` for a duplicate), then `gh issue close` with `--reason completed` for fixed and `--reason "not planned"` otherwise. A `close` with no `closeReason` (for example his own `change` to close) is closed as not planned, and the comment says so. |
