> **Status note (added 2026-08-30 when this doc was restored to `main`).**
> This is the original 2026-07-28 brainstorm, preserved because issues #139 and
> #140 cite it as their design doc and it had been living only on the unmerged
> branch behind PR #150. Two parts are now out of date:
>
> - **A1's dependency** — the cook flow it waits on has since shipped (`CookModal`,
>   `/api/ai/recipes/cook`), so the seeded-recipe deep link is unblocked.
> - **A3's interaction** — "Used it up / Tossed" as plain buttons everywhere is
>   superseded by #140's 2026-08-12 update, which specifies a graduated swipe
>   (partial drag reveals, tap or past-threshold drag commits) for normal cards
>   and visible buttons only on expiring/expired ones. #140 is the current spec;
>   this doc is the rationale behind it.
>
> The problem framing and Direction B (proactive, folded into #43) are unchanged.
